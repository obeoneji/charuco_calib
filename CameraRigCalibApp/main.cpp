...
int main(int argc, char** argv) {
	...
	std::regex regex_pattern(".*rig_([0-9]{3})\\\\cam_([0-9]{3})\\\\frame000000.*$");
	...
	size_t root_camera_idx = getCameraRigRootCameraId(rig_cameras);
	size_t num_cams = rig_cameras.size() - start_cam_id;
	root_camera_idx -= start_cam_id;
	
	// skip the lower cameras
